Feature: Trait Create
    ...
    Scenario: Switching scale class from ordinal to nominal and back to ordinal - form behavior
        ...
        When user sets "<third scale category>" in second scale field on traits list page
        When user selects 'x' button for third scale category
        Then user can sees 'Yes, remove' button in modal box
        When user selects 'Yes, remove' button in modal box
        When user sets "<fifth scale category>" in third scale field on traits list page
        When user selects "Nominal" in scale dropdown on traits list page
        When user selects "Ordinal" in scale dropdown on traits list page
        Then user can see "1" in the first Ordinal field on traits list page
        Then user can see "<first scale category>" in first Scale field on traits list page
        Then user can see "3" in the second Ordinal field on traits list page
        Then user can see "<third scale category>" in second Scale field on traits list page
        Then user can see "5" in the third Ordinal field on traits list page
        Then user can see "<fifth scale category>" in third Scale field on traits list page
